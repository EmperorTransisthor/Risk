...
void send_all(int sendfd, SA *sadest, socklen_t salen, time_t time, int randm, char* buff)
{
	char		line[MAXLINE];		/* hostname and process ID */
	
	//---------------------------------------------------------
	snprintf(line, sizeof(line), "GAME^%jd^%d^%s", time, randm, buff);
	printf("new multicast request!\n");
	//---------------------------------------------------------
...
void recv_all(int recvfd, socklen_t salen, int* readytosend, long* time, int* randm)
{
	int					n;
	char				line[MAXLINE+1];
	socklen_t			len;
...
int ServMULTICAST(int* multicast_pid) {
	...
	char nameServ[] = "Ryzyko.pl najlepsza rozrywka pod Sloncem";
	char buff1[] = "224.0.0.90";
	char buff2[] = "3456";
	char buff3[] = "enp0s3";
	...
	fcntl( *multicast_pid, F_SETFL, fcntl(*multicast_pid, F_GETFL) | O_NONBLOCK); // non-blocking read
	int info = 0;
	
	while(1){
		recv_all(recvfd, salen, &rdtosend, &id_time_cli, &id_rand_cli);
		if(read( *multicast_pid, &info, sizeof(int)) > 0){
			if(info) {
				printf("Multicast closed\n");
				exit(0);
			}
		}
		if(rdtosend){
			send_all(sendfd, sasend, salen, id_time_cli, id_rand_cli, nameServ);
			rdtosend = 0;
		}
	}
}
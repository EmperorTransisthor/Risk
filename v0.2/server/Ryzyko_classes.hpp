...
class World {
    ...
    */
    ...
    public:
        ...
        void numberOfPlayers_Setter2(int input);
        ...
        // Functions
        void playerConv(PlayerServ playerFrom, Player* player);
};
...
class Sector : public Region
{
    ...
    public:
        ...
        // Constructors
        Sector(int numberOfPlayersFromServ, char* arrayOfNicks[MAXPLAYERS]);
        Sector(std::vector <PlayerServ>  playersFromServ);
        ...
        void squadEngagement(std::string attackingSector, std::string attackedSector, int numberOfAttackingSquads);     // Squad engagement for singleplayer
        void squadEngagement(std::string attackingSector, std::string attackedSector, int numberOfAttackingSquads, struct PipeHelper* pipeC);     // Squad engagement for multiplayer

        int playerErasure ();                            // singleplayer                                                                   // checks if player has been defeated
        int playerErasure (struct PipeHelper* pipeC);    // multiplayer
        void regionChecker();
        void regionChecker(struct PipeHelper* pipeC);                                                                       // checks, if player is in possession of a whole region
...
//Player playerConv(PlayerServ playerFrom);           // We use this functions to translate server player to Risk player
